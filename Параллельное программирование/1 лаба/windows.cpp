...
DWORD WINAPI multiply_matrices_windows(LPVOID arg) {
...
void  multiply_matrices(int num_threads,
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        if (i == num_threads - 1) {
            thread_data[i].end_index = MATRIX_SIZE * MATRIX_SIZE;
        }

        threads[i] = CreateThread(nullptr, 0, multiply_matrices_windows, &thread_data[i], 0, nullptr);
    }
...
int main() {
    ...
    for (int size : sizes) {
        ...
        double boost_windows = 0;
        double boost_openmp = 0;
        ...
        auto time_windows = end_windows - start_windows;
        std::cout << "Умножение с помощью Windows Threads:  для размера " << size << ": "
            << std::chrono::duration<double>(time_windows).count() << " seconds\n";
        ...
        auto time_openmp = end_openmp - start_openmp;
        std::cout << "Умножение с помощью OpenMP для размера " << size << ": "
            << std::chrono::duration<double>(time_openmp).count() << " seconds\n";

        auto start_seq = std::chrono::high_resolution_clock::now();
        multiply_matrices_seq(C_seq, A, B, size);
        auto end_seq = std::chrono::high_resolution_clock::now();
        auto time_seq = end_seq - start_seq;
        std::cout << "Умножение последовательно для размера " << size << ": "
            << std::chrono::duration<double>(time_seq).count() << " seconds\n";

        boost_windows = time_seq / time_windows;
        boost_openmp = time_seq / time_openmp;

        std::cout << "Ускорение OpenMP: " << boost_openmp << "\n";
        std::cout << "Ускорение  Windows Threads: " << boost_windows << "\n";
        
        
        output << "Size, Windows Threads Time,OpenMP Time,Sequential Time\n";
        output << size << "," << std::chrono::duration<double>(time_windows).count() << "," << std::chrono::duration<double>(time_openmp).count() << "," << std::chrono::duration<double>(time_seq).count() << "\n";

        
        output2 << "Size, Boost Windows Threads Time, Boost OpenMP Time\n";
